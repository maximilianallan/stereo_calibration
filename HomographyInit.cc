...
bool HomographyInit::Compute(vector<HomographyMatch> vMatches, double dMaxPixelError, SE3<> &se3SecondFromFirst)
{
  ...
  for(unsigned int i=0; i<mvMatches.size(); i++)
    if(IsHomographyInlier(mm3BestHomography, mvMatches[i])) //ie look at whether the homography error is bigger than the largest allowed error
...
//in mlesac this is done with 4 map points that are all different, possibly done with more in other occasions
Matrix<3> HomographyInit::HomographyFromMatches(vector<HomographyMatch> vMatches)
{
  unsigned int nPoints = vMatches.size();
  assert(nPoints >= 4);
  /* Four points required as in x' = Hx, H is a 3x3 matrix that need to be solved. 9 elements --> defined up to a
   * scale factor give 8 degrees of freedom (define 8 elements in terms of one constant element) then, as 4 points have
   * accounts for 2 degrees of freedom so 4 points give the 8 constraints needed to determine H. see p91 of h&z
   */
  int nRows = 2*nPoints;
  if(nRows < 9)
    nRows = 9; // at least 4 points, create a matrix of (=>9,9)
  ...
  for(unsigned int n=0; n<nPoints; n++)
    {
      ...
      // (x,y) is the point in one camera plane, (u,v,) is the point in the other
      // [u v]T = H [x y]T
      m2Nx9[n*2+0][0] = x;
      m2Nx9[n*2+0][1] = y;
      m2Nx9[n*2+0][2] = 1;
      ...
      m2Nx9[n*2+1][6] = -x*v;
      m2Nx9[n*2+1][7] = -y*v;
      m2Nx9[n*2+1][8] = -v; //fills all rows
    }
  ...
  Vector<9> vH = svdHomography.get_VT()[8]; // after computing svd of A, hte smallest singluar value is given by the last column
...
void HomographyInit::BestHomographyFromMatches_MLESAC()
{
  ...
  for(int nR = 0; nR < 300 ; nR++)
      ...
      for(int i=0; i<4; i++)
	{
	  ...
	  while(!isUnique)
	    {
	      n = rand() % mvMatches.size(); //random number between 0 and the number of matched points, each pick (in anIndicies) must be unique
	      isUnique =true;
      ...
      for(int i=0; i<4; i++)
	vMinimalMatches.push_back(mvMatches[anIndices[i]]); // we now have 4 unique map points to do mlesac
